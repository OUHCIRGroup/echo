...
const RatePrompt = (props) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // NEW: Update the local state immediately if callback is provided
      if (props.onRatingSubmitted) {
        props.onRatingSubmitted(props.promptID, docRef.id, array[rating]);
      }
  ...
  return (
    <div className="fixed flex flex-col bg-[#e3e3e3] py-12 px-16 h-fit rounded-xl min-w-[30rem] max-h-[85%] overflow-auto space-y-4">
      <div className="flex flex-col text-black space-y-4 max-w-[28rem]">
        <h1 className="text-lg">
          How did the system perform compared to your expectation under the most
          recent prompt?
        </h1>
        <div className="flex flex-col pl-8 pr-16 justify-between space-y-2">
          {/* Array to generate the radio buttons */}
          {Array.from({ length: 3 }).map((_, index) => {
            return (
